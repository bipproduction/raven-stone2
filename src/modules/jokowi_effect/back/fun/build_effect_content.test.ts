...
import {
    ...
    pickRandom,
    randomInt,
} from "./build_effect_content"
import { EFFECT_ASPECT_TITLE, EFFECT_ASPECTS, EFFECT_NARRATIVE, EffectAspect } from "./effect_phrases"
...
describe("build_effect_content", () => {
    ...
    it("buildEffectAspectParagraph menghasilkan satu paragraf HTML dengan judul tebal", () => {
        const html = buildEffectAspectParagraph("FIGURE_INFLUENCE", lcg(5))
        expect(html.startsWith("<p>")).toBe(true)
        expect(html.endsWith("</p>")).toBe(true)
        expect(html).toContain(`<strong>${EFFECT_ASPECT_TITLE.FIGURE_INFLUENCE}.</strong>`)
    })

    it("tiap paragraf berisi minimal MIN dan tak melebihi MAX kalimat naratif", () => {
        const random = lcg(7)
        for (let i = 0; i < 300; i++) {
            for (const aspect of EFFECT_ASPECTS) {
                const sentences = splitSentences(narrativePart(buildEffectAspectParagraph(aspect, random)))
    ...
    it("jumlah kalimat bervariasi antar paragraf (bukan selalu sama)", () => {
        ...
        for (let i = 0; i < 200; i++) {
            counts.add(splitSentences(narrativePart(buildEffectAspectParagraph("CONTINUITY_PERCEPTION", random))).length)
        }
    ...
    it("kalimat dalam satu paragraf tidak duplikat", () => {
        const random = lcg(11)
        for (let i = 0; i < 200; i++) {
            const sentences = splitSentences(narrativePart(buildEffectAspectParagraph("DIGITAL_ECHO", random)))
            expect(new Set(sentences).size).toBe(sentences.length)
        }
    })

    it("buildEffectContent memuat seluruh aspek, dipisah newline", () => {
        const content = buildEffectContent(lcg(13))
        const lines = content.split("\n")
        expect(lines.length).toBe(EFFECT_ASPECTS.length)
        lines.forEach((line) => {
            expect(line.startsWith("<p>")).toBe(true)
            expect(line.endsWith("</p>")).toBe(true)
        })
        for (const aspect of EFFECT_ASPECTS) {
            expect(content).toContain(`<strong>${EFFECT_ASPECT_TITLE[aspect]}.</strong>`)
        }
    })
    ...
    it("menghasilkan konten berbeda antar generate (acak)", () => {
        ...
        for (let i = 0; i < 20; i++) outputs.add(buildEffectContent(random))
