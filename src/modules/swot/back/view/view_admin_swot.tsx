...
export default function ViewAdminSwot({ params, kandidat, datatable, datadownload }: { params: any, kandidat: any, datatable: any, datadownload: any }) {
    ...
    async function onChooseCandidate({ idCandidate }: { idCandidate: any }) {
        setKandidat(idCandidate)
    }

    function onProsses() {
        if (isKandidat == null) return toast("Silahkan pilih provinsi", { theme: "dark" })
        router.replace("/dashboard-admin/swot?candidate=" + isKandidat)
    }
                ...
                >
                        ...
                        <Paper shadow="xs" p="xl">
                            <Stack>
                                <Select
                                    ...
                                    label={"Kandidat"}
                                    searchable
                                    onChange={(val) => onChooseCandidate({ idCandidate: val })}
                                />
                                <Button
                                    bg={"gray"}
                                    onClick={() => onProsses()}
                    ...
                    >
                        <Box
                            ...
                            onClick={() => router.push("/dashboard-admin/swot/upload")}
                        >
