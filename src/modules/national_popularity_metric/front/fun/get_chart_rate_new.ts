...
export default async function funGetChartRateNew({ paslon, startDate, endDate }: { paslon: any, startDate: any, endDate: any }) {
    ...
    const getDate = await prisma.paslonPopularityNew.groupBy({
        by: ['dateEmotion'],
        orderBy: {
            dateEmotion: 'asc'
        },
    ...
    for (let i = 0; i < getDate.length; i++) {
        ...
        const getDateTime = await prisma.paslonPopularityNew.groupBy({
            by: ['timeEmotion'],
            orderBy: {
                timeEmotion: 'desc'
            },
    ...
    for (let i = 0; i < dataDateTime.length; i++) {
        ...
        if (!_.isUndefined(dataDateTime[i].time)) {
            const find = await prisma.paslonPopularityNew.findMany({
                where: {
                    idPaslon: Number(paslon),
    ...
    // const data = await prisma.paslonPopularityNew.findMany({
    //     where: {
    //         idPaslon: Number(paslon),
    //         dateEmotion: {
    //             gte: new Date(startDate),
