...
export async function GET() {
   ...
   async function sendWA(kategori: string) {
      ...
      if (kategori == 'sukses') {
         kalimat = '*RAVEN* : copy data paslon *sukses*'
      } else if (kategori == 'already-exist') {
         kalimat = '*RAVEN* : copy data paslon *gagal* karena data hari ini telah tersedia'
      } else if (kategori == 'data-unavailable') {
         kalimat = '*RAVEN* : copy data paslon *gagal* karena tidak ada data dihari sebelumnya'
      }
   ...
   // data emosi kemarin
   const dataEmotionYesterday = await prisma.paslonEmotion.findMany({
      where: {
   ...
   if (dataEmotionYesterday.length > 0) {
      ...
      const dataCandidateToday = await prisma.paslonEmotion.groupBy({
         where: {
      ...
      const dataFilter = dataEmotionYesterday.filter((x: any) => !dataCandidateToday.some((y: any) => y.idPaslon === x.idPaslon));
